  ...
  Future<void> _launchUrl() async {
    ...
    if (value == null || value.displayValue == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('No valid QR code scanned')));
      return;
    }
...
class _ZebraCreateFormPageState extends State<ZebraCreateFormPage> {
  ...
  Future<void> _publishForm() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Failed to publish form: $e')));
    }
  }
  ...
  Future<void> _saveQrCode() async {
    try {
      ...
      final storageRef = FirebaseStorage.instance.ref().child(
        'qr_codes/$_formId.png',
      );
      await storageRef.putData(pngBytes);
      final downloadUrl = await storageRef.getDownloadURL();

      // Save download URL to Firestore
      await FirebaseFirestore.instance.collection('forms').doc(_formId).update({
...
class _QuestionWidgetState extends State<QuestionWidget> {
  ...
  Future<void> _pickMedia() async {
    ...
    if (pickedFile != null) {
      // Upload to Firebase Storage
      final storageRef = FirebaseStorage.instance.ref().child(
        'form_media/${const Uuid().v4()}_${pickedFile.name}',
      );
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Media uploaded: $downloadUrl')));
    }
  }
